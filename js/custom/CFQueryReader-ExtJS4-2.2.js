...
Ext.define('ColdFusion.data.reader.CFQueryReader',{
	...
	readRecords: function(data) {
		var me = this, obj, error, mapit = true;
		// If the query is the root, then wrap it
		if (data.DATA && data.COLUMNS) {
			var tmp = new Object();
			tmp["QUERY"] = Ext.Object.merge({},data);
			data = tmp;
		}
		...
		if (mapit) {
			obj = me.getQuery(data);
			try {
				if (Ext.isObject(obj)) {
					// Check the 'query' value, to ensure that it is a ColdFusion query object
					if((obj.DATA && Ext.isArray(obj.DATA)) && (obj.COLUMNS && Ext.isArray(obj.COLUMNS))) {
						...
						if (me.model) {
							me.model.setFields(fields, me.idProperty);
    ...
    buildExtractors : function() {
        ...
        if (me.root) {
            var root = me.root.split(".");
        	if (root[root.length-1] === "DATA") {
        		root.pop();
        	}
        	root = root.join(".");
        	me.query = root;
            me.getQuery = me.createAccessor(me.query);
        	me.root = (root.length) ? root + ".DATA" : "DATA";
